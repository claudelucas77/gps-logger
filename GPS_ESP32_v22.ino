...
// BP enregistrement : pin 32 et GND
...
/// Autres variables
    ...
    int charge_bat_RAW = 0;                                   /// Valeur RAW de la charge batterie
    float charge_bat = 0;                                       /// Valeur de la charge batterie (Volts)
    int charg_bat_pourcent = 0;                               /// Valeur charge en %
...
/// Bouton REC
  pinMode(32,INPUT_PULLUP);                                        /// Entrée du BP qui lance l'enregistrement

/// Charge Batterie
  pinMode(35,INPUT);                                                /// Entrée tension de la batterie
...
void affichage() {
 ...
 // Serial.println("500ms");
  ...
  afficher(1,12,"     ",1);
  afficher(1,12, String(charg_bat_pourcent), 1) ;
  afficher(20,12, "%", 1) ;
  // afficher(20,20, String(temp_debug),1) ;
  // afficher(1,20,String(BP_rec),1);
...
void charge_batterie() {

charge_bat_RAW = analogRead(35);                             /// Lecture de l'entrée de la charge batterie
charge_bat = ((charge_bat_RAW * 3.3 ) /4096 ) *2 ;           //// Mise à l'échelle en Volts (il ya un pont diviseur de tension sur le montage donc on multiplie par 2 la tension lue)

float norm = (charge_bat - 3) / 1 ;                        /// Normalisation de la charge entre 0 et 1 (Lipo 0% = 3V et 100% = 4V)

charg_bat_pourcent = norm * 100 ;                             /// Mise à l'échelle en % de charge (Lipo 0% = 3.0V et 100% = 4.0V)


if (fm500ms) {
///      Serial.println(charge_bat_RAW);
///      Serial.println(charge_bat);
///      Serial.println(norm);
///      Serial.println(charg_bat_pourcent);
